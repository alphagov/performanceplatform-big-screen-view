...
module.exports = function (slideContainer) {
  ...
  return dashboard.getConfig('carers-allowance').
    fin(function (dashboardConfig) {
      ...
      var html = '';
      var introSlideTemplate = _.template('<h1>Performance data for <%= title %></h1><p class="data-from">Data from www.gov.uk/performance</p>'),
        slideTemplate = _.template('<h2><%= dashboardTitle %></h2><p class="content"><span class="big-number"><%= data %></span> <span class="title"><%= title.toLowerCase() %></span><p class="dashboard-slug">www.gov.uk/performance/<%= dashboardSlug %></p>');
